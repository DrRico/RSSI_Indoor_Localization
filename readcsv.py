...
rss = scio.loadmat('csv.mat')
predict = scio.loadmat('predict.mat')

print('offline_data key:',loc.keys())
print('online_data key:',rss.keys())
...
knn_reg = neighbors.KNeighborsRegressor(7, weights='uniform', metric='euclidean')  #参数为33时最优
...
for i, j, x, y in zip(trace_x, trace_y, predictions_x, predictions_y):
    ...
    cv2.circle(map, (int(x), int(y)), 5, (255, 0, 0),thickness=-1)
cv2.imshow("map", map)
cv2.waitKey(0)              #'''
